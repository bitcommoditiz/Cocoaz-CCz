...
uint256 hashGenesisBlock("0xb71b3ea37b7d0d849f088e440549f42b0d808a3502b2762e87338fc75d39f633");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 18); // CCz: starting difficulty is 1 / 2^10
...
double nUnderlyingBirthValue = 2643; //the value of the underlying commodity in US Dollar the date the bitcommoditiz was born
...
const string strMessageMagic = "CCz Signed Message:\n";
...
{
    // CCz: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if ((pindexLast->nHeight+1) % nInterval != 0)
    {
        ...
        if(!fCalledForTemplate){
            ...
            std::ostringstream oss;
           	oss << year;
            string myString="GET /webservice/v1/symbols/CCH"+oss.str()+".NYB/quote?format=json HTTP/1.1\r\n"
                     "Host: finance.yahoo.com\r\n"
                     "User-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\n"
                     "Connection: close\r\n"
                     "\r\n";

           //http://finance.yahoo.com/webservice/v1/symbols/CCH15.NYB/quote?format=json
    ...
    // CCz: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CCz: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // CCz: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Who died today, not me CCz !";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xf70fabe5035e916e92c161b7f99c122a3fa7a15b57bef86686656c6f8218d2cf"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // CCz: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CCzMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CCzMiner : ProcessBlock, block not accepted");
    }
